...
export default (props) => {
    const {topText, bottomText, wrapping} = props;
    return (
      <div className="panel panel-default taskDualtext">
        <div className="panel-heading">{"texte chiffré et déchiffré"}</div>
...
export const renderDualText = function (topText, bottomText, wrapping) {
  ...
  const lineHeader = (
    <div key={0} className="dualtext-label">
  ...
  for (let iCell = 0; iCell < topCells.length; iCell += 1) {
    if (iCell === wrappingCol) {
      if (line) {
        lines.push(
          <div key={lines.length} className="dualtext-line">
            {line}
          </div>
        );
      }
