...
const BareSubstTarget = props => {
  const onLock = event => {
    ...
    const {targetSymbol, source} = props;
    props.onLock(source, targetSymbol);
  };
  ...
  let el = (
    <div className={classnames(classes)}>
      ...
      {frequency && (
        <span
          className="adfgx-subst-freq"
          title={(frequency * 100).toFixed(1) + "%"}
        >
          <span style={{height: (frequency * barScale).toFixed(1) + "px"}} />
        </span>
      )}
    </div>
  );
...
export default props => {
  const onDrop = (dragSource, dragTarget) => {
    const {targetAlphabet} = props.substitution;
    ...
    props.onSwapPairs(key1, value1, key2, value2);
  };
  ...
  const {cipherFrequencies, targetFrequencies, onLock} = props;
  const barScale = targetFrequencies
    ? 42 / Math.max.apply(null, targetFrequencies)
    : 0;

  const {sourceAlphabet, targetAlphabet, mapping} = props.substitution;

  const renderSubstCell = (sourceRank, stat) => {
    ...
    return (
      <div
        ...
        className={classnames([
          "adfgx-subst-pair",
          getQualifierClass(targetCell.qualifier)
        ])}
      >
        <div className="adfgx-subst-src">
          {stat && (
            <span className="adfgx-subst-freq" title={`${stat.percentage}%`}>
              <span
                style={{
                  height: (stat.frequency * barScale).toFixed(1) + "px"
                }}
              />
            </span>
          )}
        ...
        <Target
          ...
          onDrop={onDrop}
          frequency={stat && targetFrequencies[targetCell.rank]}
          barScale={barScale}
          onLock={onLock}
          locked={isLocked}
        />
      ...
      <div className="panel-body">
        ...
        <div className="grillesSection">
          <div
            className={classnames([
              "adfgx-subst",
              targetFrequencies && "adfgx-have-freqs"
            ])}
          >
            ...
            {cipherFrequencies
              ...
              : mapping.map((_, sourceRank) =>
                  renderSubstCell(sourceRank, null)
                )}
          </div>
        </div>
      </div>
