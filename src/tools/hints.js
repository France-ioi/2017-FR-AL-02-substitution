...
export default props => {
  const onShowHintRequest = function (event) {
    ...
    props.onShowHintRequest({rank, source});
  };

  const onCloseHintRequest = function () {
    props.onShowHintRequest(null);
  };
  ...
  const renderHintRequest = () => {
    const {task, substitution, hintRequest} = props;
    const maximumScore = task.version === 1 ? 150 : 100;
    const hintCost = 10;
    const highestPossibleScore = Math.max(
      0,
      maximumScore - Object.keys(task.hints).length * hintCost
    );
    const inCipher = hintRequest.source === "cipher";
    const alphabet = inCipher
      ? substitution.sourceAlphabet
      : substitution.targetAlphabet;
    ...
    return (
      <div className="hintsDialog">
        <p>
          ...
          {inCipher ? " dans le texte chiffré" : " dans le texte clair"}
        </p>
        <p>
  ...
  const {substitution, hintRequest} = props;
  const {mapping, reverse, sourceAlphabet, targetAlphabet} = substitution;

  const renderDecipherPair = (sourceSymbol, sourceRank) => {
    ...
    return (
      <div key={sourceRank} className={classnames(pairClasses)}>
        ...
        <div
          className={classnames(tgtCharClasses)}
          onClick={isHint ? null : onShowHintRequest}
          data-rank={sourceRank}
          data-in={"cipher"}
        >
          <span className="adfgx-subst-char">
  ...
  const renderCipherPair = (targetSymbol, targetRank) => {
    ...
    return (
      <div key={targetRank} className={classnames(pairClasses)}>
        <div
          className={classnames(srcCharClasses)}
          onClick={isHint ? null : onShowHintRequest}
  ...
  return (
    <div className="panel panel-default task-hints">
      <div className="panel-heading">{"indices"}</div>
      <div className="panel-body">
        {hintRequest && renderHintRequest()}
        <div>
          <p className="hug-bottom text-bold">
            {"Substitution de déchiffrage :"}
          </p>
